...
하단 1차 탭은 다음과 같이 고정한다(Phase 3 / Option C 기준, 2025-11 개정).

1. 홈 탭 -> `MK-00 상권 홈` (현재 활성 상권의 메인. 상권 미선택 시 `M-00 지자체 포털`)
2. 가게 탭 -> 가게 목록/지도 (`MK-00`의 참여 가게 목록 화면, ST-00 진입)
3. 커뮤니티 탭 -> 멤버 커뮤니티 피드(UC-301/302) + 단골 소식(UC-303) 진입 허브
4. 마이 탭 -> `U-00-1 내 활동` + 상단 "내 혜택" 카드(`U-00-3 혜택 허브`)와 운영자 진입(`O-00`) 포함

보충 규칙:
- `G-00`은 초기 진입(지자체 선택) 전용 화면으로 운영하며, 탭에서 직접 진입하지 않는다.
- `M-00 지자체 포털`은 홈 탭 칩의 "상권 변경" 동선으로만 진입한다(상권 미선택 시 자동 표시 제외).
- 사용자가 `M-00`에 진입한 이후에는 하단 네비게이션으로 `G-00` 복귀 경로를 제공하지 않는다.
- 혜택과 운영자 진입은 별도 탭으로 두지 않고 마이 탭 내 1뎁스 카드로 통합한다(탭 수 4개 유지).
- 상단 헤더의 "상권 컨텍스트 칩"은 홈/가게/커뮤니티/단골 소식 화면에서 노출한다.
- 칩을 탭하면 **상권 전환 바텀시트**가 열린다. 시트 내 구성:
  1. 현재 지자체(예: 강동구)에 속한 상권 목록 — 현재 선택 상권은 체크 표시
  2. 상권 카드 탭 → 해당 상권 홈으로 즉시 전환(화면 이탈 없음)
  3. 하단 고정 버튼 "상권 전체 보기" → `M-00 지자체 포털`로 이동(전체 상권 탐색 화면)
- 바텀시트는 화면 이탈 없이 상권을 빠르게 전환하는 용도이며, M-00은 "전체 탐색·상세 비교" 용도로 역할을 분리한다.
